...
# from torch.utils.tensorboard import createConfusionMatrix
...
def model_eval_multitask(
    ...
    paraphrase_accuracy, para_y_true, para_y_pred, para_sent_ids = (
        model_eval_paraphrase(paraphrase_dataloader, model, device)
    )
    sts_corr, sts_y_true, sts_y_pred, sts_sent_ids = model_eval_sts(
        sts_dataloader, model, device
    )
    sentiment_accuracy, sst_y_true, sst_y_pred, sst_sent_ids = model_eval_sentiment(
        sentiment_dataloader, model, device
    )

    if tensorboard and writer is not None:
        writer.add_figure(
            "Confusion matrix SST", confusion_matrix(sst_y_true, sst_y_pred), epoch
        )
        writer.add_figure(
            "Confusion matrix Para",
            confusion_matrix(para_y_true, para_y_pred),
            epoch,
        )
        # writer.add_figure("Confusion matrix STS", confusion_matrix((np.round(sts_y_true,1) * 10).astype(int), (np.round(sts_y_pred,1) * 10).astype(int)), epoch)
...
def test_model_multitask(args, model, device):
    ...
    prediction_folder = args.logdir + "/predictions"
    if not os.path.exists(prediction_folder):
        os.makedirs(prediction_folder)

    sst_test_data, num_labels, para_test_data, sts_test_data, etpc_test_data = (
    ...
    # Mean accuracy
    print(
        f"Mean accuracy :: {np.mean([dev_sentiment_accuracy, dev_paraphrase_accuracy, dev_sts_corr]) :.5f}"
    )
